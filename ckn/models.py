...
class UnsupCKNet(CKNet):
    ...
    def unsup_cross_val(self, data_loader, test_loader=None, n_sampling_patches=500000,
        ...
        # Convertim la numpy doar train, care este cu siguranță definit
...
class UnsupCKNetTinyImageNet(UnsupCKNet):
    def __init__(self, filters, kernel_sizes, subsamplings, sigma):
        # nclass=200 and image_size=64 for Tiny-ImageNet
        super(UnsupCKNetTinyImageNet, self).__init__(
            10, 3, filters, kernel_sizes, subsamplings,
            kernel_args_list=sigma, fit_bias=False, maxiter=1000, image_size=64)


class UnsupCKNetSVHN(UnsupCKNet):
    def __init__(self, filters, kernel_sizes, subsamplings, sigma):
        # For SVHN: 10 classes, 3 channels, image_size=32
        super(UnsupCKNetSVHN, self).__init__(
            10, 3, filters, kernel_sizes, subsamplings,
            kernel_args_list=sigma, fit_bias=False, maxiter=5000, image_size=32)



class UnsupCKNetSTL10(UnsupCKNet):
    def __init__(self, filters, kernel_sizes, subsamplings, sigma):
        """
        Modelul CKN nesupravegheat pentru STL-10:
        - 10 clase
        - 3 canale (RGB)
        - 96x96
        - Folosește parametrii (filters, kernel_sizes, subsamplings, sigma)
          ca la celelalte (CIFAR/SVHN).
        """
        super(UnsupCKNetSTL10, self).__init__(
            nclass=10,
            in_channels=3,
            out_channels_list=filters,
            kernel_sizes=kernel_sizes,
            subsamplings=subsamplings,
            kernel_args_list=sigma,
            fit_bias=False,
            maxiter=5000,    # poți schimba
            image_size=96    # foarte important pentru dimensionare
        )
